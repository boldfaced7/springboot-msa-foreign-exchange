...
class CheckFxDepositWithDelayServiceTest {

    @InjectMocks
    private CheckFxDepositWithDelayService checkFxDepositWithDelayService;
    ...
    @Test
    @DisplayName("외화 입금 지연 확인 요청 시, 외화 입금 확인 필요 이벤트가 발행되어야 한다.")
    void checkFxDepositWithDelay() {
        // given
        RequestId requestId = new RequestId(1L);
        Count count = new Count(1);
        CheckFxDepositWithDelayCommand command = new CheckFxDepositWithDelayCommand(requestId, count);

        // 1. 거래 요청 조회
        when(exchangeRequestLoader.loadExchangeRequest(requestId)).thenReturn(exchangeRequest);
        // 2. 외화 입금 확인 필요 이벤트 발행
        doNothing().when(exchangeRequest).checkingFxDepositRequired(count);
        doNothing().when(exchangeEventPublisher).publishEvents(exchangeRequest);

        // when
        checkFxDepositWithDelayService.checkFxDepositWithDelay(command);

        // then
        verify(exchangeRequestLoader).loadExchangeRequest(requestId);

        verify(exchangeRequest).checkingFxDepositRequired(count);
