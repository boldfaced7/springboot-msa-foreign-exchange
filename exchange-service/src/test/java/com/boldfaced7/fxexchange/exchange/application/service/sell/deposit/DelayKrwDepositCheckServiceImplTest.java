...
class DelayKrwDepositCheckServiceImplTest {

    @InjectMocks
    private DelayKrwDepositCheckServiceImpl delayKrwDepositCheckService;

    @Mock
    private SendKrwDepositCheckRequestPort sendKrwDepositCheckRequestPort;
    ...
    void delayKrwDepositCheck() {
        // given
        Count count = new Count(2);
        Duration expectedDelay = Duration.ofSeconds(30).multipliedBy(count.value() + 1);

        // when
        delayKrwDepositCheckService.delayKrwDepositCheck(requestId, count);

        // then
        verify(sendKrwDepositCheckRequestPort).sendKrwDepositCheckRequest(
            eq(requestId), 
    ...
    void delayKrwDepositCheckZeroCount() {
        // given
