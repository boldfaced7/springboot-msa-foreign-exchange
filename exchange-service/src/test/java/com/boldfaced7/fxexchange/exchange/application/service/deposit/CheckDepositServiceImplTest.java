...
@TestPropertySource(properties = {
        "exchange.deposit.check.max-count=3"
})
class CheckDepositServiceImplTest {
    ...
    @Test
    @DisplayName("입금 결과 조회 성공 시, 입금 ID가 추가되어야 한다")
    ...
    @Test
    @DisplayName("입금 결과 조회 실패 시, 입금 ID가 추가되지 않아야 한다")
    ...
    @Test
    @DisplayName("입금 결과 조회 실패 시 재시도 횟수가 최대값보다 작으면 지연 체크가 필요하다")
    ...
    @Test
    @DisplayName("입금 결과 조회 재시도 횟수 초과 시, 경고 메시지 발송이 필요하다")
    void checkDeposit_RetryExceeded() {
        // given
        when(exchangeRequestLoader.loadExchangeRequest(requestId)).thenReturn(exchangeRequest);
        doThrow(new RuntimeException("Network error")).when(depositResultChecker).loadDepositResult(
                eq(exchangeRequest),
        ...
        // when & then
        assertThatThrownBy(() -> checkDepositService.checkDeposit(requestId, new Count(3), Direction.BUY))
                .isInstanceOf(RuntimeException.class);
    ...
    @Test
    @DisplayName("입금 결과 조회 시 count가 0인 경우 전달하는 람다를 검증한다")
    void checkDeposit_LambdaVerification_WhenCountIsZero() {
        ...
        verify(exchangeRequest).delayingDepositCheckRequired(Count.zero().increase());
    }

    @Test
    @DisplayName("입금 결과 조회 시 count가 최댓값을 초과한 경우 전달하는 람다를 검증한다")
    void checkDeposit_LambdaVerification_WhenCountExceedsMax() {
        // given
        when(exchangeRequestLoader.loadExchangeRequest(requestId)).thenReturn(exchangeRequest);
        doReturn(successResult).when(depositResultChecker).loadDepositResult(
                eq(exchangeRequest),
        ...
        // when
        checkDepositService.checkDeposit(requestId, new Count(3), Direction.BUY);
