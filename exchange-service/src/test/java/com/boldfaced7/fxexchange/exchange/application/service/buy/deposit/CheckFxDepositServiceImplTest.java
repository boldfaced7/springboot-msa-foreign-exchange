...
class CheckFxDepositServiceImplTest {

    @InjectMocks
    private CheckFxDepositServiceImpl checkFxDepositService;

    @Mock
    private LoadFxDepositResultPort loadFxDepositResultPort;
    ...
    @Test
    @DisplayName("외화 입금 성공 시 입금 ID가 추가되고 환전(외화 구매) 완료 이벤트가 발행되어야 한다")
    void checkFxDepositSuccess() {
        // given
        // 0. 도메인 객체 설정
        ...
        when(loadFxDepositResultPort.loadFxDepositResult(exchangeId)).thenReturn(successResult);
        ...
        // when
        checkFxDepositService.checkFxDeposit(requestId, Count.zero());

        // then
        verify(exchangeRequestLoader).loadExchangeRequest(requestId);
        verify(loadFxDepositResultPort).loadFxDepositResult(exchangeId);
    ...
    @Test
    @DisplayName("외화 입금 실패 시 원화 출금 취소 필요 이벤트가 발행되어야 한다")
    void checkFxDepositFailure() {
        // given
        // 0. 도메인 객체 설정
        when(exchangeRequest.getExchangeId()).thenReturn(exchangeId);
        ...
        // 1. 외화 입금 결과 조회
        when(exchangeRequestLoader.loadExchangeRequest(requestId)).thenReturn(exchangeRequest);
        when(loadFxDepositResultPort.loadFxDepositResult(exchangeId)).thenReturn(failureResult);
    ...
    @Test
    @DisplayName("외화 입금 결과 조회 실패 시 재시도 횟수가 최대값보다 작으면 지연 체크 필요 이벤트가 발행되어야 한다")
    void checkFxDepositRetry() {
        // given
        // 0. 도메인 객체 설정
        when(exchangeRequest.getExchangeId()).thenReturn(exchangeId);
        ...
        // 1. 외화 입금 결과 조회
        when(exchangeRequestLoader.loadExchangeRequest(requestId)).thenReturn(exchangeRequest);
        when(loadFxDepositResultPort.loadFxDepositResult(exchangeId))
                .thenThrow(new RuntimeException("Network error"));

        // 2. 지연 체크 필요 이벤트 발행
        doNothing().when(exchangeRequest).delayingFxDepositCheckRequired(new Count(1));
        doNothing().when(exchangeEventPublisher).publishEvents(exchangeRequest);

        // when & then
        assertThatThrownBy(() -> checkFxDepositService.checkFxDeposit(requestId, Count.zero()))
                .isInstanceOf(RuntimeException.class);

        verify(exchangeRequestLoader).loadExchangeRequest(requestId);
        verify(loadFxDepositResultPort).loadFxDepositResult(exchangeId);

        verify(exchangeRequest).delayingFxDepositCheckRequired(new Count(1));
        verify(exchangeEventPublisher).publishEvents(exchangeRequest);
    }

    @Test
    @DisplayName("외화 입금 결과 조회 실패 시 재시도 횟수가 최대값이면 경고 메시지가 발송되어야 한다")
    void checkFxDepositMaxRetry() {
        // given
        // 0. 도메인 객체 설정
        ...
        // when & then
        assertThatThrownBy(() -> checkFxDepositService.checkFxDeposit(requestId, new Count(3)))
                .isInstanceOf(RuntimeException.class);
