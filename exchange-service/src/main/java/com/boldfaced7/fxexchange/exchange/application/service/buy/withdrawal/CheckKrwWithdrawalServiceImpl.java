...
public class CheckKrwWithdrawalServiceImpl implements CheckKrwWithdrawalService {

    private final static int MAX_COUNT = 3;

    private final LoadKrwWithdrawalResultPort loadKrwWithdrawalResultPort;
    ...
    @Override
    public void checkKrwWithdrawal(RequestId requestId, Count count) {
        var requested = exchangeRequestLoader.loadExchangeRequest(requestId);
        var withdrawn = loadKrwWithdrawalResult(requested, count);

        markEvents(requested, withdrawn);
        exchangeEventPublisher.publishEvents(requested);
    }

    private WithdrawalResult loadKrwWithdrawalResult(ExchangeRequest requested, Count count) {
        try {
            return loadKrwWithdrawalResultPort.loadKrwWithdrawalResult(
    ...
    private void handleRetry(ExchangeRequest requested, Count count) {
        if (count.isSmallerThan(MAX_COUNT)) {
            ...
            requested.delayingKrwWithdrawalCheckRequired(count.increase());
            exchangeEventPublisher.publishEvents(requested);
        } else {
